...
input int            SessionsToCount         = 20;               // SessionsToCount: Number of sessions to count Market Profile.
...
input sessions_to_draw_rays ShowMedianRays    = All;           // ShowMedianRays: draw previous median rays.
...
input int              ProminentMedianPercentage      = 80;    // Percentage of Median TPOs out of total for a Prominent one.
...
int OnInit()
{
    ...
    // PATCH: when the developing-line features are off, suppress their plots
    // entirely. MT5 zero-fills buffer cells on new data; 0.0 is a valid price
    // to a DRAW_LINE plot, which rendered as full-height vertical bands.
    if (!EnableDevelopingPOC)
    {
               ...
               )
{
    ...
    if ((rates_total - prev_calculated > 1) || (NeedToRestartDrawing))
    {
        ...
        // PATCH: initialize ALL plot buffers unconditionally. The original only
        // initialized the POC pair (and only when EnableDevelopingPOC was on),
        // leaving VAH/VAL buffers holding 0.0 on fresh bars -- a valid price to
        // the renderer, which drew full-height vertical bands to zero.
        ArrayInitialize(DevelopingPOC_1, EMPTY_VALUE);
        ArrayInitialize(DevelopingPOC_2, EMPTY_VALUE);
        ArrayInitialize(DevelopingVAH_1, EMPTY_VALUE);
        ArrayInitialize(DevelopingVAH_2, EMPTY_VALUE);
        ArrayInitialize(DevelopingVAL_1, EMPTY_VALUE);
        ArrayInitialize(DevelopingVAL_2, EMPTY_VALUE);
    }
