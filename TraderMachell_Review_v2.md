...
- **Graphics** (only meaningful on `d.isLast()`, `global: true` + stable keys):
  ...
  - **INDICATOR STATE SURVIVES TIMEFRAME CHANGES (community-confirmed + live 2026-08-10)**: graphics-module indicators keep stale state/values across a chart timeframe change (until re-save/re-add). A surviving mirror carries (index, timestamp) pairs at the OLD bar spacing, displacing every t-du emission by the timeframe ratio. Consequence: runtime timeframe-change detection with a full self-reset is mandatory (`wrapper.js` `_checkTimeframe`: elementSize re-read + finer-bar-spacing detector; `[timeframe changed - reloading]` while the mirror refills).
